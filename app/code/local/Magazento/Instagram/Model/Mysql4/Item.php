...
class Magazento_Instagram_Model_Mysql4_Item extends Mage_Core_Model_Mysql4_Abstract {

    protected function _construct() {
        $this->_init('instagram/item', 'item_id');
    }
   ...
   protected function _beforeSave(Mage_Core_Model_Abstract $object) {
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object) {
        ...
        $this->_getWriteAdapter()->delete($this->getTable('instagram/item_page'), $condition);
        $this->_getWriteAdapter()->delete($this->getTable('instagram/item_store'), $condition);
        $this->_getWriteAdapter()->delete($this->getTable('instagram/item_category'), $condition);
        if ($object->getData('in_products')) $this->_getWriteAdapter()->delete($this->getTable('instagram/item_product'), $condition);
...
//        exit();        
        ...
        foreach ((array) $products as $product) {
            ...
            $this->_getWriteAdapter()->insert($this->getTable('instagram/item_product'), $productArray);
        }        
        ...
        foreach ((array) $object->getData('stores') as $store) {
            ...
            $this->_getWriteAdapter()->insert($this->getTable('instagram/item_store'), $storeArray);
        }
        ...
        foreach ((array) $object->getData('categories') as $category) {
            ...
            $this->_getWriteAdapter()->insert($this->getTable('instagram/item_category'), $categoryArray);
        }
        ...
        foreach ((array) $page as $page) {
            ...
            $this->_getWriteAdapter()->insert($this->getTable('instagram/item_page'), $pageArray);
        }
    ...
    protected function _afterLoad(Mage_Core_Model_Abstract $object) {
        ...
        $select = $this->_getReadAdapter()->select()
                        ->from($this->getTable('instagram/item_store'))
        ...
        //CATEGORY 
        $select = $this->_getReadAdapter()->select()
                        ->from($this->getTable('instagram/item_category'))
                        ->where('item_id = ?', $object->getId());
        if ($data = $this->_getReadAdapter()->fetchAll($select)) {
            $categoryArray = array();
            foreach ($data as $row) {
        ...
        //PRODUCT 
        $select = $this->_getReadAdapter()->select()
                        ->from($this->getTable('instagram/item_product'))
                        ->where('item_id = ?', $object->getId());
        if ($data = $this->_getReadAdapter()->fetchAll($select)) {
            $productArray = array();
            foreach ($data as $row) {
        ...
        //PAGE
        $select = $this->_getReadAdapter()->select()
                        ->from($this->getTable('instagram/item_page'))
                        ->where('item_id = ?', $object->getId());
        if ($data = $this->_getReadAdapter()->fetchAll($select)) {
            $pageArray = array();
            foreach ($data as $row) {
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object) {
        ...
        $adapter->delete($this->getTable('instagram/item_store'), 'item_id=' . $object->getId());
        $adapter->delete($this->getTable('instagram/item_category'), 'item_id=' . $object->getId());
        $adapter->delete($this->getTable('instagram/item_product'), 'item_id=' . $object->getId());
        $adapter->delete($this->getTable('instagram/item_page'), 'item_id=' . $object->getId());
    }
